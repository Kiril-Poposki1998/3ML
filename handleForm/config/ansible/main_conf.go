...
- name: Setup VM
  ...
  tasks:
    ...
    - name: Delete docker images
      become: yes
      cron: minute="0" hour="8" weekday="1" job="docker image prune -af" state=present name=remove_images
	  
	- name: Add cronjob for removing buildx data
      cron: minute="0" hour="8" weekday="0" job="docker buildx prune -af" state=present name=remove_buidlx_data
`
